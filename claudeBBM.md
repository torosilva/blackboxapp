...
| 2026-06-06 | Branch `claude/light-test` con paleta light hardcoded en 4 pantallas para validación con Directora Fuxia | Directora pidió "fondo blanco" en feedback. Master se queda dark (riesgo regresión con otros testers); creé branch separado con build de prueba SOLO para ella. Inversión de paleta dark→light en CaptureScreen (home), LoopsScreen, SettingsScreen, EntryDetailScreen: backgrounds slate-950/900 (`#0a0f1e`/`#0f172a`/`#151B2C`/`#1e293b`) → slate-50/white (`#F8FAFC`/`#FFFFFF`), borders slate-800 (`#1E293B`/`#334155`) → slate-200 (`#E2E8F0`/`#CBD5E1`), text primary slate-100/white → slate-900 (`#0F172A`). Muted text (`#64748b`, `#94a3b8`, `#475569`) intacto — funciona en ambos. Accents (purple, indigo, red, green, sky) sin cambio. Translucent whites `rgba(255,255,255,X)` → `rgba(15,23,42,X)` para borders/highlights legibles sobre fondo claro. StatusBar `light-content` → `dark-content`. Iconos en JSX preservados como blanco cuando van sobre fondo de accent (sendBtnActive, audioPlayBtn, generateButton, sendButton, avatarLarge); convertidos a `#0F172A` cuando estaban sobre fondo neutro (header backButton, action circles). Hardcoded sin toggle — Mario decide después si merge a master o descarta. PENDIENTE MARIO: `eas build --platform ios --profile production` desde branch `claude/light-test` para TestFlight de prueba; ojo de NO subir submit a producción si falla. NO toqué: HomeScreen, ChatScreen, DashboardScreen, MapasScreen, EntriesByMapScreen, OnboardingScreen, PrivacyScreen — fuera del scope de 4 pantallas principales. |
