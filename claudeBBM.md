...
| 2026-05-26 | Polish v1.7: input MUCHO más grande + bug del "Escuchando" inline | Mario validó que bg blanco solo lo hacía DISTINTO, no más prominente — el home tiene 6 bloques pesados compitiendo (input + 3 stat cards con números grandes + REFLEJO púrpura + título rojo + 3 loops cards púrpura) y el input era 1 de 6. Eligió hacerlo MUCHO más grande. También reportó bug: el indicador "Escuchando 0:04 · toca para terminar" aparecía flotando ABAJO en pantalla cuando el botón mic estaba ARRIBA — desconexión visual. Causa del bug: estilo `recPill` tenía `position:absolute bottom:96` que era anchor del FAB de mic flotante (eliminado en Camino A v1.1) — quedó huérfano sin contexto. Fix dual en CaptureScreen.tsx: (1) textCaptureCard mucho más grande: paddingVertical 18→26, paddingHorizontal 16→20, minHeight 64→92, maxHeight 200→220, borderRadius 14→16, gap 12→14, shadow más intensa (opacity 0.35→0.4, radius 12→16, height 4→6, elevation 6→8). textCaptureInput fontSize 16→18. Edit3 icon 18→22, Mic/Send 20→24. (2) recPill movido desde fuera del ScrollView (position absolute, huérfano) a INLINE justo debajo del textCaptureCard. Cambié style: removí position absolute/bottom/right, bg con tinte rojo (rgba(239,68,68,0.08)), border rojo (rgba(239,68,68,0.3)), marginTop -12 para sentarse pegado al input, marginBottom 16. Texto rojo claro (#fca5a5). Ahora "Escuchando 0:04" aparece DONDE el usuario tapeó el mic, con visual de estado de grabación claro. |
