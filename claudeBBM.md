...
| 2026-05-26 | Fix bug del chat: stuck loading bubble forever cuando edge function se cuelga | Mario reportó: chat se "atoró" sin error visible — la burbuja de loading nunca resolvió. Diagnóstico: `ChatService.sendMessage` usa `fetch()` SIN AbortController ni timeout. Si la Edge Function `ai-chat` se cuelga (red, Anthropic API slow, edge timeout interno sin respuesta), el fetch espera infinito → setLoading queda forever → catch nunca dispara → user sin feedback. Fix de 2 archivos: (1) ChatService: AbortController + setTimeout(60_000ms) + traducción de AbortError a mensaje claro "La IA tardó demasiado en responder. Verifica tu conexión e intenta de nuevo.", (2) ChatScreen catch: detecta el mensaje de timeout y lo muestra al usuario en vez del genérico. Aplicado en mismo branch `claude/home-day-1` por decisión de Mario (estricto sería branch separado, pero pragmático para no bloquear validación del Camino A). DEUDA: el mismo patrón de fetch sin timeout existe en `semanticSearch` y posiblemente otras Edge Function calls — pendiente auditar. |
...
| 2026-05-26 | Fix v1.2 polish: quitar glow púrpura del REFLEJO + relieve real en stat cards | Mi v1.2 interpretó mal el polish visual. Mario pidió relieve animado en las STAT CARDS (las 3 de arriba). Yo en su lugar agregué shadow tinte púrpura al REFLEJO que quedó como glow horrible. Invertido en este commit: (a) reflejoCard ahora con shadow negro muy sutil (color #000, opacity 0.25, radius 6, offset 0/2, elevation 3) — el background ya tiene tinte púrpura sutil, agregar shadow del mismo color duplicaba el efecto. (b) statCard con relieve REAL: border #2D3548 (era #1E293B, más contraste), borderRadius 12→14, padding 16/14, shadow opacity 0.45 (era 0.35), radius 10, elevation 6. (c) Animación de mount separada: stat cards reciben spring scale-in (de 0.96 a 1.0, friction 6, tension 80) + fade 380ms — "aterrizan con peso". El resto (REFLEJO + loops + memorias) mantiene el fade + slide-up suave de 320ms. Ambas animaciones corren en paralelo. Lección: no fui suficientemente preciso al escribir el spec de v1.2 — debí distinguir "shadow corporativo neutro" vs "shadow de tinte que se vuelve glow". |
