...
| 2026-05-26 | Home cards consistencia: small-caps title arriba en LOOPS y MEMORIAS | El wrap visual (bg, border, shadow) de loopsModule/memModule ya coincidía con homeCard desde commit a455b4a, pero faltaba el small-caps title arriba que sí tiene ESTADO ACTUAL ("ESTADO ACTUAL" label). Sin él, las 3 secciones se veían parecidas pero no idénticas. Fix: agregué `<Text style={styles.homeCardTitle}>PRIORIDAD</Text>` arriba del loops module y `<Text style={styles.homeCardTitle}>MEMORIAS</Text>` arriba del mem module. Conservo el título emocional grande ("Lo que sigues postergando") debajo como subtítulo conversacional. REFLEJO sigue distintivo con tinte púrpura (intencionalmente). |
| 2026-05-26 | Bug crítico: heurística isLikelyIncoherent bloqueaba mensajes legítimos en español >200 chars | Aaron Porraz (Director Comercial Red Hat, tester ICP real) reportó vía Mario: "ya me sale muy seguido esto cuando le escribo a BLACKBOX" — el alert "No te entendí. Tu texto parece tener letras al azar." al escribir un mensaje perfectamente legítimo de 336 chars sobre un viaje a Guadalajara. Causa: mi bigram check (threshold ≥6) disparaba con bigramas comunes del español ("es" apareció 9 veces en el mensaje de Aaron). Cualquier mensaje >200 chars natural en español tenía 6+ repeticiones de "es"/"en"/"ar"/"de"/"la". Fix: el bigram check ahora SOLO corre para compact.length < 80 chars (donde "asdfasdf" es señal real). Para textos largos, el ratio consonante/vocal (>3) ya cubre el caso. 9/9 tests passan (4 gibberish reales bloqueados + 5 textos reales permitidos, incluyendo el de Aaron). Damage: probablemente perdió otros testers en silencio. PENDIENTE: incluir este fix en el próximo build/TestFlight YA. |
| 2026-05-26 | Feature nuevo: "Tus Mapas" con Patrones + Proyectos detectados por Claude | Spec: pantalla nueva accesible desde Centro Estratégico → CONTENIDO que muestra (1) PATRONES de user_patterns existente, (2) PROYECTOS detectados via Edge Function nueva `detect-projects` (Claude Sonnet 4.6, 2 cache breakpoints). Tap en card → EntriesByMapScreen lista filtrada. 7 archivos: migration 20260528_user_projects.sql (RLS strict, unique index case-insensitive), EF detect-projects (callClaude + parseJsonLoose, valida UUIDs contra ventana de 80 entries, ignora proyectos con <3 entradas), MapasScreen con tabs Patrones/Proyectos + cache 24h via AsyncStorage para no triggear EF en cada open, EntriesByMapScreen drill-down, SupabaseService +3 métodos (getProjects, getEntriesByIds, triggerProjectDetection), SettingsScreen item "Tus Mapas" entre Conversaciones y Dashboard, RootNavigator + types.ts registrar 2 rutas. PENDIENTE MANUAL DE MARIO: (a) `supabase db push` para crear tabla user_projects, (b) `supabase functions deploy detect-projects` para activar la EF. Sin esto, la tab Proyectos quedará vacía. La tab Patrones sí funciona inmediatamente porque usa user_patterns que ya existía. |
| 2026-05-28 | Bug crítico onboarding en blanco: array ONBOARDING_QUESTIONS mezclaba 2 versiones del merge | Aaron Porraz reportó (vía Mario): pantalla del onboarding sin pregunta visible — solo dots arriba + input vacío + botón "Continuar" gris. Causa: residuo del merge OnboardingScreen.tsx de varios turnos atrás. Cuando Mario resolvió el conflict, dejó el array `ONBOARDING_QUESTIONS` con 5 elementos MIXTOS: 3 viejos (id '1','2','4') con campos `title/subtitle/description/icon/color` que NO match el tipo `Question = { id, text, hint }`, + 2 nuevos (q3, q5) con `text/hint`. Cuando el screen renderizaba `currentQuestion.text` para los items viejos, undefined → `<Text>` vacío → pantalla en blanco. Los 3 errores TS preexistentes en lines 38/46/59 eran señal de esto pero los acepté como "preexistentes" sin investigar. Fix: reemplacé el array completo con las 5 preguntas conversational consistentes (q1-q5, todas con text/hint). Typecheck bajó de 34 → 31 errores totales — los 3 que eliminé eran exactamente las prop inválidas del array roto. Lección: cuando aparecen errores TS preexistentes en archivos que tocaste recientemente, son pista activa, no ruido — debí investigar entonces, no tras reporte de usuario. |
| 2026-05-28 | Heurística gibberish removida + version bump (v1.3.0 → v1.3.2), aplicado en working tree, NO commiteado | Spec exacto del usuario: borrar isLikelyIncoherent + bloque "No te entendí" en CaptureScreen.tsx, cambiar versión en SettingsScreen.tsx, entregar solo `git diff` sin commit ni push. Respetado. Mario decide si commitea después de revisar el diff. |
| 2026-06-06 | Feedback Directora General Fuxia: "triaja" y "línea de dolor" no existen + jerga consultora aleja al usuario real | Directora General de Fuxia (tester ICP real, no ejecutiva tech-bro) preguntó si "triaja" y "línea de dolor" son palabras reales. Honestamente: NO. "Triaja" es jerga corporativa rota (el verbo es "triar"); "línea de dolor" es traducción mal hecha de "pain point". Su feedback general: app sobre-tecnificada con jerga consultora McKinsey que aliena. Cambios en 2 frentes: (1) UI strings: "Active Loops" → "Pendientes" en LoopsScreen header, SettingsScreen items, CaptureScreen statusSub, DashboardScreen status line, HomeScreen filter label. Mantenemos "REGRESAN/HOY/RONDANDO" como metáforas claras. (2) Prompts de 5 EFs (analyze-entry, analyze-patterns, ai-chat standard+therapy, detect-projects, generate-initial-reflejo): agregada sección LENGUAJE NATURAL con lista PROHIBIDA: triaja, línea de dolor, backlog operativo, decisión binaria, propietario explícito, stakeholder, deliverable, deep dive, low hanging fruit, scope (anglicismo). Equivalentes naturales: ordena/prioriza, qué le duele, pendientes acumulados, sí o no, quién se hace cargo, involucrado, entregable, profundizar. Tareas YA generadas en DB con jerga no se reescriben — solo afecta nuevas. Mario dijo NO al fondo blanco (sprint mayor) por ahora; quedó pendiente para evaluación futura si Directora insiste. |
| 2026-06-06 | Fix profundo prompts: cambiar ROL "Ex-McKinsey" → "amigo con experiencia" + lista PROHIBIDA expandida con pseudo-clínico | Directora General Fuxia escaló feedback con 2 screenshots: (1) reflejo "Catastrofismo Anticipatorio + Fusión Emocional-Operativa" con "binarias esta", "propietario explícito", "loop de control mental" — todo jerga; (2) chat con "pipeline real" preguntando explícitamente "¿qué es pipeline real?". Mi fix anterior solo agregó palabras prohibidas, pero el ROL del prompt seguía diciendo "Consultor Estratégico Senior (Ex-McKinsey)" → la IA seguía generando jerga aunque tuviera lista negra. Fix profundo en 3 EFs (analyze-entry, analyze-patterns, ai-chat standard+therapy): ROL cambiado de "Consultor McKinsey/Coach/Terapeuta" → "amigo con experiencia en negocios, claro y directo, habla como mentor en mesa de café no como sala de junta". Estilo de voz explícito: frases cortas, sujeto-verbo-objeto, cero clínico, cero McKinsey. Lista PROHIBIDA expandida con sección PSEUDO-CLÍNICO: "catastrofismo anticipatorio", "fusión emocional-operativa", "disonancia ejecutiva", "parálisis táctica", "control mental", "sustituto de delegación". Y JERGA ampliada con: "pipeline" (en cualquier sentido), "framework"/"paradigma"/"ecosistema" (cuando no son técnicos), "claridad táctica/operativa", "ejecución personal", "binariar". Mantra final: "habla como tu mejor amiga directora de ops a las 9pm de un martes". Esto NO reescribe outputs viejos en DB — solo afecta NUEVAS generaciones a partir del deploy. |
| 2026-06-06 | Sistema de tokens semánticos + toggle dark/light funcional en master para toda la app | Mario explícitamente pidió "esto es una aplicación de altos estándares por favor deja de estar haciendo parches hazlo bien lo que cueste pero bien" — fin de la fase de hardcode light. Arquitectura completa: (1) `src/theme/tokens.ts` con tipo `ThemeTokens` semántico (bg/border/text/accent/shadow/statusBar) + paletas `darkTokens` (default) y `lightTokens` (validada con Directora). (2) `src/theme/ThemeContext.tsx` con Provider + hook `useTheme` + persistencia AsyncStorage `@bbm:themeMode`. Default dark si no hay preferencia. (3) `RootNavigator.tsx` wrapped con `<ThemeProvider>` entre SafeAreaProvider y AuthProvider. (4) Refactor de 32 archivos: cada pantalla y componente convierte su `const styles = StyleSheet.create(...)` a `const makeStyles = (tokens: ThemeTokens) => StyleSheet.create(...)`, y el componente usa `const { tokens } = useTheme(); const styles = useMemo(() => makeStyles(tokens), [tokens]);`. Sub-componentes definidos fuera del principal también suscriben useTheme. Cada hex literal mapeado a token semántico. JSX inline colors usan `tokens.X.Y`. StatusBar dinámico. (5) Toggle UI en SettingsScreen grupo AJUSTES → "Apariencia" con chips Oscuro/Claro y switch instantáneo. (6) Branch `claude/light-test` queda obsoleto (era hardcode temporal — Mario decide cuándo borrarlo). Trabajo paralelizado con 3 agentes que procesaron 28 archivos mientras yo manejé SettingsScreen + 4 principales + arquitectura. 0 errores TS nuevos en el refactor (solo quedan 7 errores preexistentes de otros sprints). PENDIENTE MARIO: `eas build --platform ios --profile production` desde master para empujar a TestFlight. Una vez validado, todos los usuarios tienen el toggle. |
| 2026-06-06 | Bump iOS buildNumber 4 → 20 para envío a TestFlight | Mario pidió mandar build 20 a App Store Connect. Único cambio: `app.json` ios.buildNumber. Commiteado y pusheado a master. Pendiente: Mario corre `eas build --platform ios --profile production --auto-submit` desde su Mac. |
